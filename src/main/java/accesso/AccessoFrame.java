...
public class AccessoFrame extends JFrame {
  ...
  private void login() {
    ...
    if (username.isEmpty() || password.isEmpty()) {
      JOptionPane.showMessageDialog(
          null,
          "Username o password non possono essere vuoti",
          "Errore",
      ...
      if (aU.findUtent(username, password)) {
        if (AccessoUtenteMain.findUtent(username, password)) {
          ...
          JOptionPane.showMessageDialog(
              null, "L'account non esistente", "Errore", JOptionPane.ERROR_MESSAGE);
        }
      } else {
        JOptionPane.showMessageDialog(
  ...
  private void registrazione() {
    ...
    JButton pulsanteConfermaRegistrazione =
        creaPulsante("Conferma Registrazione", new Color(0, 153, 0));
    gbc.gridy = 8;
    pannello.add(pulsanteConfermaRegistrazione, gbc);

    pulsanteConfermaRegistrazione.addActionListener(
        e ->
            confermaRegistrazione(
                nuovoCampoUsername, nuovoCampoPassword, nuovoCampoConfermaPassword));
  ...
  private void confermaRegistrazione(
      JTextField nuovoCampoUsername,
      JPasswordField nuovoCampoPassword,
      JPasswordField nuovoCampoConfermaPassword) {
    ...
    if (username.isEmpty() || password.isEmpty() || confermaPassword.isEmpty()) {
      JOptionPane.showMessageDialog(
          this, "Tutti i campi devono essere compilati", "Errore", JOptionPane.ERROR_MESSAGE);
      return;
    } else {
      if (!password.equals(confermaPassword)) {
        JOptionPane.showMessageDialog(
            this, "Le password non coincidono", "Errore", JOptionPane.ERROR_MESSAGE);
        return;
      } else {
        if (AccessoUtenteMain.addUtent(username, password)) {
          ...
          JOptionPane.showMessageDialog(
              null, "Account già esistente", "Errore", JOptionPane.ERROR_MESSAGE);
        }
      }
    }
