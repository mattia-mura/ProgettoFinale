...
public class AccessoUtenteMain {
  ...
  public AccessoUtenteMain() {
    ...
    if (!CartellaUtenti.exists()) {
      CartellaUtenti.mkdir();
    }
  }

  public static boolean addUtent(
      String nomeUtente,
      String password) { // controlla password e nomeUtente --> solo caratteri e numeri , no spazi
    File Utente = new File(name + nomeUtente + ".txt");
    File Utente2 = new File(name + nomeUtente + ".csv");
    BufferedWriter BW = null;
    BufferedWriter bw = null;
    if (Utente.exists() || Utente2.exists()) {
      return false;
    }
    try {
  ...
  public static boolean findUtent(String nomeUtente, String password) {
    ...
    if (Utente.exists()) {
      try (BufferedReader BW = new BufferedReader(new FileReader(Utente))) {
        ...
        if (passwordUtent.equals(password)) {
          return true;
        }
      } catch (IOException e) {
        e.printStackTrace();
  ...
  public static boolean addInfo(
      ...
      LocalDate localDate,
      double soldiInvestiti,
      double guadagno,
      int durataInvestimento) {
