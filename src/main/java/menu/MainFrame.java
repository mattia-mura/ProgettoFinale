...
public class MainFrame extends JFrame {
  ...
  public static Portafoglio getPortafoglio() {
    return portafoglioSolidi;
  }
  ...
  public Investimenti getInvestimenti(int i) {
    return investimenti.get(i);
  }
  ...
  public void transizioni() {
    Transizioni frameTransizioni = new Transizioni(nomeee);
  ...
  public void salvaEEsci() {
    ...
    if (AccessoUtenteMain.addInfo(
        AccessoFrame.getDatiUtente()[0], portafoglioSolidi, contoVirtuale, data, -1, -1, -1)) {
      JOptionPane.showMessageDialog(this, "Dati salvati.");
    } else {
      JOptionPane.showMessageDialog(
          this, "Dati Non salvati.", "Informazione", JOptionPane.WARNING_MESSAGE);
    }

    dispose();
  }

  public static boolean depositPreleva(
      int scelta, double money, Portafoglio portafoglio, ContoBanca contoBancario) {
    if (scelta == 1) {
      if ((money > portafoglio.getSchei()) || (money <= 0)) {
        return false;
      }
      portafoglio.decrementaSchei(money);
      contoBancario.aumentaSaldo(money);
    } else {
      if ((money > contoBancario.getSaldo()) || (money <= 0)) {
        return false;
      }
