...
public class InvestiFrame extends JFrame {
  ...
  public InvestiFrame() {
    ...
    breveDurata.addActionListener(
        e -> {
          durataInvestimento = 1;
          selezionaDurata();
        });

    mediaDurata.addActionListener(
        e -> {
          durataInvestimento = 2;
          selezionaDurata();
        });

    lungaDurata.addActionListener(
        e -> {
  ...
  private void indirizzoInvestimenti() {
    ...
    if (durataInvestimento == -1 || livelloRischio == -1) {
      JOptionPane.showMessageDialog(
          this,
          "Seleziona durata e rischio prima di confermare l'investimento.",
          "Errore",
  ...
  public double confermaInvestimento() {
    ...
    if (durataInvestimento == -1 || livelloRischio == -1) {
      JOptionPane.showMessageDialog(
          this,
          "Seleziona prima durata e livello di rischio.",
          "Errore",
          JOptionPane.ERROR_MESSAGE);
      return 0.0;
    }
    ...
    try {
      ...
      if (soldi > 0 && soldi < MainFrame.getContoVirtuale()) {
        ...
        JOptionPane.showMessageDialog(
            this, "L'importo deve essere maggiore di 0.", "Errore", JOptionPane.ERROR_MESSAGE);
      }
  ...
  */
  ...
  public double investimentoo(
      double capitale, int probabilitaGuadagno, int rangeGuadagno[], int rangePerdita[]) {

    int probabilita = (int) (Math.random() * 100 + 1);

    if (probabilita <= probabilitaGuadagno) {
      int percentualeGuadagno =
          (int) (Math.random() * (rangeGuadagno[1] - rangeGuadagno[0] + 1) + rangeGuadagno[0]);
      capitale *= 1 + (double) percentualeGuadagno / 100;
    } else {
      int percentualePerdita =
          (int) (Math.random() * (rangePerdita[1] - rangePerdita[0] + 1) + rangePerdita[0]);
